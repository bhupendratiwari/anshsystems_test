...
<html>
    <head>
        <meta charset="UTF-8">
        <title>{{ $page_title . ' - ' . SITE_NM }}</title>
        ...
        <script src="{{ base_url(SITE_JS) }}/jquery-2.2.0.min.js"></script>
        ...
        <script>
            ...
            language_list = {!! count($CI->language_list) > 1 ? json_encode($CI->language_list) : '[]' !!};
    ...
    <body class="hold-transition skin-blue sidebar-mini">
        <div class="wrapper">
             @if($CI->config->item('show_admin_header')) @include('common.header') @endif

            <div class="wrapper row-offcanvas row-offcanvas-left">
                @if($CI->config->item('show_admin_sidebar'))  @include('common.sidebar-left') @endif
                @yield('pg_content')                
            </div>
            @if($CI->config->item('show_admin_footer')) @include('common.footer') @endif
        </div>
        ...
        <script src="{{ base_url(SITE_JS) }}/plugins/iCheck/icheck.min.js" type="text/javascript"></script>
         ...
         <script type="text/javascript" src="{{ base_url(SITE_JS.'/bootstrap-confirmation.js') }}"></script>
        ...
        @if(lang('validatorLang') != 'en')
        <script src="{{ base_url(SITE_JS.'validator/localization/messages_'.lang('validatorLang').'.js') }}" type="text/javascript"></script>
        @endif
